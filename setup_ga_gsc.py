...
def find_existing_property(token):
    ...
    MUST paginate: the GA4 Admin API caps pageSize (default 50). The account
    has 60+ properties, so a single un-paginated page silently misses existing
    ones and this script creates a DUPLICATE every run (PFRCPR 2026-05-16).
    """
    page_token = None
    while True:
        url = (
            "https://analyticsadmin.googleapis.com/v1beta/properties"
            f"?filter=parent:{GA4_ACCOUNT}&pageSize=200"
        )
        if page_token:
            url += f"&pageToken={page_token}"
        r = rq.get(url, headers=gh(token), timeout=15)
        if not r.ok:
            return None
        body = r.json()
